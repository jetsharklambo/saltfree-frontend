...
const LockButton = styled.button<{ locked?: boolean }>`
  background: ${({ locked }) => locked ? 'rgba(255, 193, 7, 0.2)' : 'rgba(255, 255, 255, 0.1)'};
  border: 1px solid ${({ locked }) => locked ? 'rgba(255, 193, 7, 0.4)' : 'rgba(255, 255, 255, 0.2)'};
  border-radius: 8px;
  width: 36px;
  height: 36px;
  display: flex;
  align-items: center;
  justify-content: center;
  color: ${({ locked }) => locked ? '#ffc107' : 'rgba(255, 255, 255, 0.7)'};
  cursor: pointer;
  transition: all 0.2s ease;
  
  &:hover {
    background: ${({ locked }) => locked ? 'rgba(255, 193, 7, 0.3)' : 'rgba(255, 255, 255, 0.15)'};
    transform: scale(1.05);
  }
`;
              ...
              {splits.map((split, index) => {
                ...
                return (
                  <div key={index} style={{ 
                    ...
                    borderRadius: '12px',
                    border: isActive ? '1px solid rgba(120, 119, 198, 0.3)' : '1px solid rgba(255, 255, 255, 0.1)'
                  }}>
                    {/* Header */}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                      <span style={{ fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.6)' }}>Fine adjust:</span>
                      <SmartInput
                        type="number"
                        step="0.1"
                        min="0"
